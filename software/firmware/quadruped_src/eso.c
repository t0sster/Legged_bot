...
float ESO_2(ESO *eso_in,float y,float u,float T)    
{
	...
	eso_in->beta0=1/(eso_in->h);
	eso_in->beta1=1/(30*pow(eso_in->h,2));

	e=eso_in->e=my_deathzoom1(eso_in->z[0]-y,eso_in->eso_dead);
	fe=fal(e,0.5,eso_in->h0);
	fe1=fal(e,0.25,eso_in->h0);
	
	eso_in->z[0]+=eso_in->h*		(eso_in->z[1]-eso_in->beta0*e+eso_in->b0*u);
	eso_in->z[1]+=-eso_in->h*		 eso_in->beta1*fe;
  if(eso_in->b0!=0)
		return eso_in->disturb=eso_in->z[1]/eso_in->b0;
	else
		return 0;
}

float ESO_2N(ESO *eso_in,float v,float y,float u,float T,float MAX,float ero_px4)             // v�ǿ���ϵͳ�����룬y�ǿ���ϵͳ�������������ESO��u��ADRC�����������
{
	float e=0,fe,fe1;
	eso_in->h=eso_in->h0;
//********  eso  *************
...
//-----------------------------POS-ESO--------------------------------
float OLDX_ESO_2N_POS(ESO *eso_in,float v,float y,float u,float T,float MAX,u8 for_high)             // v�ǿ���ϵͳ�����룬y�ǿ���ϵͳ�������������ESO��u��ADRC�����������
{
	float e=0,fe,fe1;
	eso_in->h=eso_in->h0;
	e=eso_in->e=my_deathzoom1(eso_in->z[0]-y,eso_in->eso_dead);
