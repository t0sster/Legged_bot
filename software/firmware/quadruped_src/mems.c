...
void IMU_Data_Prepare(float T)
{
    ...
    MEMS_Data_Offset(); //校准函数
		...
		mems.Gyro_I16.x=lis3mdl_cov.Gyro_I16.x ;
		mems.Gyro_I16.y=lis3mdl_cov.Gyro_I16.y ;
...
//10 170 4056
    ...
    mems.Gyro_deg.z = mems.Gyro.z *TO_ANGLE;
		...
		LIS_CalOffset_Mag(T);
		float temp_mag[3];
		DigitalLPF(lis3mdl.Mag_Adc.x,&temp_mag[0],FLT_MAG,T) ;
		DigitalLPF(lis3mdl.Mag_Adc.y,&temp_mag[1],FLT_MAG,T) ;
		DigitalLPF(lis3mdl.Mag_Adc.z,&temp_mag[2],FLT_MAG,T) ;
    ...
    mems.Mag.z = mems.Mag_Val.z;
		...
		static float temp_mago[3];
		if(module.hml_imu_o){
			DigitalLPF(mag_outer[0],&temp_mago[0],FLT_MAG,T) ;
			DigitalLPF(mag_outer[1],&temp_mago[1],FLT_MAG,T) ;
			DigitalLPF(mag_outer[2],&temp_mago[2],FLT_MAG,T) ;
			mems.Mag_Adc_o.x=temp_mago[0];mems.Mag_Adc_o.y=temp_mago[1];mems.Mag_Adc_o.z=temp_mago[2];
			mems.Mag_Valo.x=(mems.Mag_Adc_o.x - mems.Mag_Offseto.x)*mems.Mag_Gaino.x;
			mems.Mag_Valo.y=(mems.Mag_Adc_o.y - mems.Mag_Offseto.y)*mems.Mag_Gaino.y;
			mems.Mag_Valo.z=(mems.Mag_Adc_o.z - mems.Mag_Offseto.z)*mems.Mag_Gaino.z;
		
			mems.Mago.x = mems.Mag_Valo.x;
			mems.Mago.y = mems.Mag_Valo.y;
			mems.Mago.z = mems.Mag_Valo.z;
		}
		#if 0
		mems.Acc_rt=mems.Acc;
		mems.Gyro_deg_rt=mems.Gyro_deg;
		mems.Mag_rt=mems.Mag;
		#else
		...
		converRT_float(mems.Mag.x,mems.Mag.y,mems.Mag.z,
		&mems.Mag_rt.x,&mems.Mag_rt.y,&mems.Mag_rt.z);	
		#endif
}
