...
void mavlink_init(void){//机器人类型和协议类型选择
...
static NOINLINE void send_heartbeat(mavlink_channel_t chan)
{
    ...
    uint8_t state;
//	  switch(state){//mode_oldx.flow_hold_position){
//			case 0:
...
static NOINLINE void send_attitude(mavlink_channel_t chan)
{
    mavlink_msg_attitude_send(
        chan,
        ++buf_mavlink[1],//millis(),
        Roll/57.3,//ahrs.roll,
        Pitch/57.3,//ahrs.pitch,
        Yaw/57.3,//ahrs.yaw,
...
static void NOINLINE send_location(mavlink_channel_t chan)
{
    ...
    if(ABS(nav.lat)<5||ABS(nav.lon)<5){
			nav.lat=39.9626679;//39.92;
		  nav.lon=116.3040411;//114.46;
		}
	   mavlink_msg_global_position_int_send(
        chan,
        1,//millis(),
        nav.lat*10000000,//current_loc.lat,                // in 1E7 degrees
        nav.lon*10000000,//current_loc.lng,                // in 1E7 degrees
        nav.pos_n[2]*1000,//g_gps->altitude * 10,             // millimeters above sea level
        nav.pos_n[2]*1000,//(current_loc.alt - home.alt) * 10,           // millimeters above ground
        nav.spd_n[0]*100,//g_gps->ground_speed * rot.a.x,  // X speed cm/s
        nav.spd_n[1]*100,//g_gps->ground_speed * rot.b.x,  // Y speed cm/s
        nav.spd_n[2]*100,//g_gps->ground_speed * rot.c.x,
        9);//g_gps->ground_course);          // course in 1/100 degree
			mavlink_msg_local_position_ned_send(chan, 1,
  		 nav.pos_n[1],  nav.pos_n[0],  nav.pos_n[2], nav.spd_n[1], nav.spd_n[0], nav.spd_n[2]);
}

//#include "rc.h"
static void NOINLINE send_rc(mavlink_channel_t chan){
// mavlink_msg_rc_channels_raw_send(chan, 1, 0, Rc_Get.ROLL-1000+1024, Rc_Get.PITCH-1000+1024, 
//	Rc_Get.YAW-1000+1024, Rc_Get.THROTTLE-1000+1024,Rc_Get.AUX1-1000+1024, Rc_Get.AUX2-1000+1024, Rc_Get.AUX3-1000+1024, Rc_Get.AUX4-1000+1024, 255);
}
...
static void NOINLINE send_ahrs(mavlink_channel_t chan)
{
    ...
    mavlink_msg_ahrs_send(
        chan,
        ++buf_mavlink[8],//omega_I.x,
        ++buf_mavlink[9],//omega_I.y,
        ++buf_mavlink[10],//omega_I.z,
        1,
        0,
        ++buf_mavlink[11],//ahrs.get_error_rp(),
        ++buf_mavlink[12]);//ahrs.get_error_yaw());
}
