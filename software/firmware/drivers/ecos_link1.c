...
{ 
  ...
  TxMessage.RTR=0;		  // 消息类型为数据帧，一帧8位
	if(kp>KP_MAX[motor_id]) kp=KP_MAX[motor_id];
		else if(kp<KP_MIN[motor_id]) kp=KP_MIN[motor_id];
	if(kd>KD_MAX[motor_id] ) kd=KD_MAX[motor_id];
		else if(kd<KD_MIN[motor_id]) kd=KD_MIN[motor_id];	
	if(pos>POS_MAX[motor_id])	pos=POS_MAX[motor_id];
		else if(pos<POS_MIN[motor_id]) pos=POS_MIN[motor_id];
	if(spd>SPD_MAX[motor_id])	spd=SPD_MAX[motor_id];
		else if(spd<SPD_MIN[motor_id]) spd=SPD_MIN[motor_id];
	if(tor>T_MAX[motor_id])	tor=T_MAX[motor_id];
		else if(tor<T_MIN[motor_id]) tor=T_MIN[motor_id];

  kp_int = float_to_uint_rv(kp, KP_MIN[motor_id], KP_MAX[motor_id], 12);
  kd_int = float_to_uint_rv(kd, KD_MIN[motor_id], KD_MAX[motor_id], 9);
	pos_int = float_to_uint_rv(pos, POS_MIN[motor_id], POS_MAX[motor_id], 16);            
  spd_int = float_to_uint_rv(spd, SPD_MIN[motor_id], SPD_MAX[motor_id], 12);
  tor_int = float_to_uint_rv(tor, T_MIN[motor_id], T_MAX[motor_id], 12);	
		...
		if(ack_status==1)//response frame 1 ----->rx here
		{
			...
			if(id>=0&&id<10){
				if(sel==0){
					...
					rv_motor_msg[id].current_actual_float=uint_to_float_rv(cur_int,I_MIN[id],I_MAX[id],12);
					rv_motor_msg[id].temperature=(RxMessage->Data[6]-50)/2;
					
					data_can_rv_anal(&motor_chassis[id],
					rv_motor_msg[id].angle_actual_rad,//输出轴
...
/// limit data to be within bounds ///    
	...
	float t_ff = LIMIT(set_t,-ptr->max_t,ptr->max_t); //最终发送的力矩前馈
	
	if(EN_MIT_PID_INNER==0||ptr->param.usb_cmd_mode==2)//纯力矩前馈、电流模式
	   kp=kd=v_des=0;
	
	if(ptr->param.control_mode==1)//转速模式
	{
		kp=0;
	}
  //send_motor_ctrl_cmd(uint16_t motor_id,float kp,float kd,float pos,float spd,float tor)
	send_motor_ctrl_cmd(ptr->param.id, kp, kd, p_des, v_des, t_ff);

  return 0;		
}

char data_can_sample_only_rv(motor_measure_t *ptr)//仅采集发送
{
	u8 canbuft1[8];
/// limit data to be within bounds ///  
	int q_flag=1;
